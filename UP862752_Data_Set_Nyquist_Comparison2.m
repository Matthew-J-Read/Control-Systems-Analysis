...
function UP862752_Data_Set_Nyquist_Comparison2()
dbdata = [6.02 5.52 4.61 3.41 1.86 -1.24 -2.83 -5.78 -6.02 -7.96];   % dB gain
pdata = [-7.20 -16.64 -32.40 -45.82 -63.44 -80.43 -99.23 -111.34...  % phase
         -110.69 -124.23];
...
G = tf(1058, [1 59.8 529]);       % Second order transfer function
...
plot(x,y,'x','MarkerSize',12)     % Mark data points with a cross
title('Nyquist Data Comparison','FontSize',14) % Create title
xlabel('Real Axis','FontSize',12)              % Create X-axis label          
ylabel('Imaginary Axis','FontSize',12)         % Create Y-axis label
axis([-0.5 2.1 -1.7 0.4])                      % Set axis limits
end
% ----------------------------------------------------------------------%